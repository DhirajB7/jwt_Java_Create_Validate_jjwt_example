...
public class Create {
  ...
  public static String createJwtSignedHMAC() throws Exception {

    {
      ...
      String jwtToken = Jwts.builder().claim("name", "Jane Doe").claim("email", "jane@example.com").setSubject("jane")
          .setId(UUID.randomUUID().toString()).setIssuedAt(Date.from(now))
          .setExpiration(Date.from(now.plus(5l, ChronoUnit.MINUTES))).signWith(privateKey).compact();
  ...
  private static PrivateKey getPrivateKey() throws Exception {
    String rsaPrivateKey = "MIIEvwIBADANBgkqhkiG9w0BAQEFAASCBKkwggSlAgEAAoIBAQDK7c0HtOvefMRM"
        ...
        + "YENBqXLZnWGfIo0vAIgniJ6OD67+1hEQtbenhSfeE8Hou2BnFOTajUxmYgGm3+hx" + "h8TPOvfHATdiwIm7Qu76gHhpzQ==";
